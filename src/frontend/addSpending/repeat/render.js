...
for (let i = 0; i < options.length; i++) {
    ...
    if (option.className)
        option.innerHTML = fileHandler.resolveLanguageCode(option.className, navigator.language)
}

let intervallSelecct = document.getElementById("repeatIntervall")
let intervallOptions = [
    
]

intervallOptions.forEach(item => {
    console.log(item["value"].valueOf())
})
